    ...
    <div class="mt-12 flex justify-center gap-3">
        @foreach($testimonials as $index => $testimonial)
            <button @click="goTo({{ $index }})"
                    ...
                    :class="currentIndex === {{ $index }} ? 'w-8 bg-primary' : 'w-2 bg-muted dark:bg-muted hover:bg-border dark:hover:bg-border'"></button>
        @endforeach
    </div>
</div>
