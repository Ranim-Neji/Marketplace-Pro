...
<body class="min-h-screen font-['Inter'] text-foreground bg-background antialiased transition-colors duration-500 overflow-x-hidden relative">
    ...
    <div class="flex min-h-screen relative z-10">
        ...
        <main class="flex-1 flex flex-col min-w-0 max-h-screen overflow-y-auto custom-scrollbar">
            ...
            <header class="h-24 bg-card/40 backdrop-blur-xl border-b border-border sticky top-0 z-30 flex items-center justify-between px-10">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <form method="POST" action="{{ route('logout', absolute: false) }}">
                        @csrf
                        <button class="h-12 px-6 rounded-2xl border border-warning dark:border-warning/30 text-warning text-[10px] font-black uppercase tracking-widest hover:bg-warning hover:text-white dark:hover:bg-warning dark:hover:text-white transition-all italic">
                            Logout
                        </button>
                    </form>
                </div>
            ...
            <div class="p-10">
                @if(session('success'))
                    <div class="mb-10 p-6 rounded-3xl bg-emerald-50 dark:bg-primary/10 border border-primary dark:border-primary/30 flex items-center gap-4 animate-slide-up">
                        <div class="h-10 w-10 bg-primary rounded-xl flex items-center justify-center text-white">
                            <i class="fa-solid fa-check"></i>
                        </div>
                        <div class="text-[10px] font-black text-primary dark:text-primary uppercase tracking-widest italic">{{ session('success') }}</div>
                    </div>
                @endif
